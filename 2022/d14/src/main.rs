...
fn simulate_sand_part2(points: &HashSet<(i32, i32)>, max_height: i32) -> Option<(i32, i32)> {
    let mut sand_position: (i32, i32) = (500, 0);
...
fn part_2() {
    ...
    let max_height = all_points.iter().max_by_key(|x| x.1).unwrap().1 + 2;
    let mut counter = 0;
    loop {
        match simulate_sand_part2(&all_points, max_height) {
            Some(p) => {
                if p.0 == 500 && p.1 == 0 {
                    break;
                }

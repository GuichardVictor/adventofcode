...
const N_ROUNDS_PART_2: usize = 10_000;
...
fn part_2() {
    ...
    let modulo: usize = monkeys.iter().map(|m| m.divisor_test).product();
    for _ in 0..N_ROUNDS_PART_2 {
        for i in 0..monkeys.len() {
            let mut monkey = &mut monkeys[i];
            monkey.inserted += monkey.items.len();
            ...
            iterator.drain(..).for_each(|item| {
                let new_value = (monkeys[i].operation)(item) % modulo;
